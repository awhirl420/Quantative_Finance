...
def backtest(start_date, end_date, hold_days, strategy, data, weight='average', benchmark=None, stop_loss=None,
    ...
    ((equality/equailty.cummax()-1)*100).plot(lengend=True)
...
def portfolio(stock_list, money, data, lowest_fee=20, discount=0.6, add_cost=10):
    ...
    while (money / len(stock_list)) < (lowest_fee - add.cost) * 1000 / 1.425 / discount:
        stock_list = stock_list[stock_list != stock_list.max()]
    print('after conidering fee', len(stock_list))
